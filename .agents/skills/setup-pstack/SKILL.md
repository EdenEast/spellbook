...
Write `~/.local/share/spellbook/ptack-model.mdc`, an always-applied rule that sets pstack's model per role. The skills read it and fall back to their inline defaults when a line is absent, so this is an override layer, not a requirement.
...
The default role-to-model mapping is the rule shape shown in step 5 below. If `~/.local/share/spellbook/ptack-model.mdc` already exists, read it and treat its values as the current choices. Otherwise start from those defaults.
...
Write `~/.local/share/spellbook/ptack-model.mdc` with `alwaysApply: true` and one line per role, using the same labels poteto-mode uses. Overwrite the whole file so re-runs stay idempotent. Shape:
